...
dtex_load_epe(uint8_t* buf) {
	...
	struct ej_sprite_pack* ej_pkg = ej_pkg_import(ptr, body_sz, tex, maxid, unpack_sz);
	return ej_pkg;
}

struct dtex_rrp* 
dtex_load_rrp(uint8_t* buf, size_t sz) {
...
struct dtex_rrr* 
dtex_load_rrr(uint8_t* buf, size_t sz) {
	uint32_t cap = buf[0] | buf[1]<<8 | buf[2]<<16 | buf[3]<<24;
...
struct dtex_b4r* 
dtex_load_b4r(uint8_t* buf, size_t sz) {
	uint32_t cap = buf[0] | buf[1]<<8 | buf[2]<<16 | buf[3]<<24;
