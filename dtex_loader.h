...
struct dtex_package* dtex_load_pkg(struct dtex_loader*, const char* name, const char* filepath, int format, float scale, int lod);
void dtex_preload_texture(struct dtex_loader*, struct dtex_package* pkg, int idx, float scale);
void dtex_load_texture(struct dtex_loader*, struct dtex_package*, int idx, float scale, bool create_by_ej);
